...
def show_similarity_interactive(image_path_a: str, image_folder_path_b: str, load_size: int = 224, layer: int = 11,
    ...
    for image_path in sorted(os.listdir(image_folder_path_b)):
        ...
        while len(pts) == 1:
            ...
            center = ((x_descs_coor - 1) * stride + stride + patch_size // 2 - .5,
                      (y_descs_coor - 1) * stride + stride + patch_size // 2 - .5)
            patch = plt.Circle(center, radius, color='g')
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        show_similarity_interactive(args.image_a, args.image_b_folder, args.load_size, args.layer, args.facet, args.bin,
