...
class ViTExtractor:
    ...
    def __init__(self, model_type: str = 'dino_vits8', stride: int = 4, model: nn.Module = None, device: str = 'cuda'):
        ...
        self.p = self.model.module.patch_embed.patch_size[0] if isinstance(self.model.module.patch_embed.patch_size,
                                                                           tuple) else self.model.module.patch_embed.patch_size
    ...
    def _extract_features(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key') -> List[
        torch.Tensor]:
        """
                    ...
                    for i in range(y - kernel_size, y + kernel_size + 1, kernel_size):
                        for j in range(x - kernel_size, x + kernel_size + 1, kernel_size):
                            ...
                            if 0 <= i < self.num_patches[0] and 0 <= j < self.num_patches[1]:
                                bin_x[:, part_idx * sub_desc_dim: (part_idx + 1) * sub_desc_dim, y, x] = avg_pools[
                                                                                                             k][
                                                                                                         :, :, i, j]
