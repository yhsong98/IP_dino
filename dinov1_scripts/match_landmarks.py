...
def show_similarity_interactive(image_path_a: str, image_folder_path_b: str, landmarks, num_ref_points: int, load_size: int = 224, layer: int = 11,
    ...
    for image_path in sorted(os.listdir(image_folder_path_b)):
        ...
        result = classify_landmark(b_landmark_points)
        print(result)
        labels, _ = cluster_matched_points(b_landmark_points)
        visualize_clusters(image_pil_b, b_landmark_points, labels)
...
def cluster_matched_points(matched_points, eps=20, min_samples=3):
    """
...
def classify_landmark(matched_points, eps=20, min_samples=3):
    """
...
def visualize_clusters(image, matched_points, labels):
    """
    Visualizes clustered matched points on Image B.

    :param image: Image B where matches were found.
    :param matched_points: List of (x, y) coordinates.
    ...
    colors = plt.cm.get_cmap("tab10", len(unique_labels))  # Generate colors
    ...
    for label in unique_labels:
        ...
        cluster_points = np.array(matched_points)[labels == label]
        plt.scatter(cluster_points[:, 0], cluster_points[:, 1], c=[color], label=f"Cluster {label}", s=20)

    plt.axis("off")
    plt.legend()
    plt.title("Clustered Matched Points on Image B")
    plt.show()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Facilitate similarity inspection between two images.')
    parser.add_argument('--image_a', type=str, default="images/landmark_files/cat_face_source.jpg", help='Path to the first image')
    parser.add_argument('--image_b_folder', type=str, default="images/dog_rotated", help='Path to the second image.')
    ...
    parser.add_argument('--num_ref_points', default=50, type=int, help="number of reference points to show.")
    parser.add_argument('--landmark_file', default='images/landmark_files/cat_face_landmarks.csv', type=str, help="landmarks file.")
