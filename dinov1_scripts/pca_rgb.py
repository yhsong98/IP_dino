...
def cluster_similaritise(similarities, eps=0.1, min_samples=2):
    similarities = similarities.reshape(-1, 1)  # Reshape for clustering
    clustering = DBSCAN(eps=eps, min_samples=min_samples).fit(similarities)
    return clustering.labels_  # Cluster labels

def is_ambiguous(clustering_labels):
    """
    Determine if a point is ambiguous based on clustering labels.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root_dir', type=str, default='images/cat', help='The root dir of images.')
    parser.add_argument('--save_dir', type=str, default='images/pca/cat', help='The root save dir for results.')
    ...
    parser.add_argument('--n_components', default=10, type=int, help="number of pca components to produce.")
