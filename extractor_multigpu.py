...
class ViTExtractor:
    ...
    def __init__(self, model_type: str = 'dino_vits8', stride: int = 4, model: nn.Module = None, device: str = 'cuda'):
        ...
        self.model.eval()
        self.model = nn.parallel.DistributedDataParallel(self.model.to(self.device), device_ids=[local_rank])
        #self.model.to(self.device)
        self.p = self.model.module.patch_embed.patch_size[0] if isinstance(self.model.module.patch_embed.patch_size,tuple) else self.model.module.patch_embed.patch_size
        self.stride = self.model.module.patch_embed.proj.stride
    ...
    def patch_vit_resolution(model: nn.Module, stride: int) -> nn.Module:
        ...
        # fix the stride
        model.module.patch_embed.proj.stride = stride
    ...
    def v_preprocess(self, video_path: Union[str, Path],
        ...
        while cap.isOpened():
            ...
            if not ret:
                break
            pil_image = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))  # Convert BGR to RGB
            if load_size is not None:
                pil_image = transforms.Resize(load_size, interpolation=transforms.InterpolationMode.LANCZOS)(pil_image)
            prep = transforms.Compose([
                transforms.ToTensor(),
            ...
            prep_img = prep(pil_image)[None, ...]
            frames.append((prep_img, pil_image))
        cap.release()
        return frames
    ...
    def _register_hooks(self, layers: List[int], facet: str) -> None:
        ...
        """
        for block_idx, block in enumerate(self.model.module.blocks):
    ...
    def extract_saliency_maps(self, batch: torch.Tensor) -> torch.Tensor:
        ...
        """
        assert self.model_type == "dinov2_vits14", f"saliency maps are supported only for dino_vits model_type."
